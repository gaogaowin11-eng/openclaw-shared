...
module.exports = {
  ...
  toJson: function (src_excel_file, dest_dir, head, separator) {
    ...
    ensureDirectorySync(dest_dir);
    console.log("parsing excel:", src_excel_file);
    var excel = xlsx.parse(src_excel_file);
    var workbookName = path.basename(src_excel_file, path.extname(src_excel_file));
    _toJson(excel, dest_dir, head, workbookName);
  }
};
 ...
 */
function _toJson(excel, dest, head, workbookName) {
  var workbookOutput = {};
  for (var i_sheet = 0; i_sheet < excel.worksheets.length; i_sheet++) {
    ...
    if (sheet.name.indexOf('@') == -1 && sheet.name[0] !== "#") {
      ...
      workbookOutput[sheet.name] = output;
    }
  }

  var finalOutput = JSON.stringify(workbookOutput);
  var workbookFileName = (workbookName || 'workbook') + ".json";
  var dest_file = path.resolve(dest, workbookFileName);
  fs.writeFile(dest_file, finalOutput, function (err) {
    if (err) {
