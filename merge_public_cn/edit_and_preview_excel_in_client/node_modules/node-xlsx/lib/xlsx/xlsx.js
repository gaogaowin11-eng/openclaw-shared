...
function xlsx(file, options) { 
	...
	function unescapeXML(s) { 
		return (s || '').replace(/&#(\d+);/g, function(m, code) { return String.fromCharCode(+code); }).replace(/&#x([0-9a-fA-F]+);/g, function(m, code) { return String.fromCharCode(parseInt(code, 16)); }).replace(/&amp;/g, '&').replace(/&lt;/g, '<').replace(/&gt;/g, '>').replace(/&quot;/g, '"').replace(/&#x27;/g, '\''); 
	}
		...
		while (i--) {
			...
			while (--j) { // Don't process j === 0, because s[0] is the text before the first row element
				...
				while (--k) { // Don't process l === 0, because k[0] is the text before the first c (cell) element
					...
					if (t === 'inlineStr') {
						// Handle inline strings: <is><t>value</t></is>
						var isStart = cell.indexOf('<t>');
						var isEnd = cell.indexOf('</t>');
						val = (isStart !== -1 && isEnd !== -1) ? unescapeXML(cell.substring(isStart + 3, isEnd)) : '';
					} else {
